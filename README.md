...
2. Clients that have established a secure connection will be receiving 2 of the same/similar position packets at slightly offset times due to ping. This *could* lead to players seeming to stutter back and forth at higher pings and lead to a slightly worse experience, may be especially apparent when playing close proximity or chase game modes like Tag, but I think it's a worthwhile trade off to make the game playable.
...
TLDR:
1. This mod does not fix the root issue causing the floating players, only working around the problem (SteamP2P).
...
### [Installation Video by Storm](https://www.youtube.com/watch?v=2dnf7JWObh8)
...
2. Extract/unzip the file and put its contents (the "BepInEx" and "mono" folders with the "doorstop_config.ini" and "winhttp.dll" files) in your Crab Game directory (open your Steam Library, right click Crab Game, Manage -> Browse local files).
